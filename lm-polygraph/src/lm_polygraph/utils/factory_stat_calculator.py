...
def create_stat_calculator(module_name: str, config, environment, deberta = None):
    module = import_module(module_name)
    if module_name != 'lm_polygraph.defaults.stat_calculator_builders.default_SemanticMatrixCalculator':
        return module.load_stat_calculator(config, environment)
    else:
        return module.load_stat_calculator(config, environment, deberta = deberta)
...
class FactoryStatCalculator:
    ...
    def __call__(
        self, stat_calculators_info: List[StatCalculatorContainer], deberta = None
    ) -> List[StatCalculator]:
        stat_calculators = [
